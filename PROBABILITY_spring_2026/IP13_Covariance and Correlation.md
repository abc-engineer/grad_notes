...
- 성질  
- Properties 1. $\text{Cov}(aX+b,;cY+d)=ac,\text{Cov}(X,Y)$ 여기서 $a,b,c,d$는 상수이다.
- Properties 2. $\text{Cov}(X_1+X_2,\;Y)=\text{Cov}(X_1,Y)+\text{Cov}(X_2,Y)$
- Properties 3. $\text{Cov}(X,X)=\text{Var}(X)$
- Properties 4. $\text{Cov}(X,Y)=E[XY]-\mu_X\mu_Y$
- Properties 5. $\text{Var}(X+Y)=\text{Var}(X)+\text{Var}(Y)+2\text{Cov}(X,Y)$
- Properties 6.
...
$$  
\text{Cov}(X,Y)=\iint (x-\mu_X)(y-\mu_Y)f_X(x)f_Y(y),dx,dy  
$$
...
$$  
\text{Cov}(X,Y)=\sum_{i=1}^{n}\sum_{j=1}^{m} p(x_i,y_j)(x_i-\mu_X)(y_j-\mu_Y)  
$$

- 또한 성질 $\text{Cov}(X,Y)=E[XY]-\mu_X\mu_Y$를 이용하면  

$$  
\text{Cov}(X,Y)=\left(\sum_{i=1}^{n}\sum_{j=1}^{m} p(x_i,y_j)x_i y_j\right)-\mu_X\mu_Y  
$$
...
$$  
\text{Cov}(X,Y)=\int_c^d\int_a^b (x-\mu_X)(y-\mu_Y)f(x,y),dx,dy  
$$

- 또한 성질 $\text{Cov}(X,Y)=E[XY]-\mu_X\mu_Y$를 이용하면  

$$  
\text{Cov}(X,Y)=\left(\int_c^d\int_a^b xy,f(x,y),dx,dy\right)-\mu_X\mu_Y  
$$
...
$$  
\text{Cov}(X,Y)=\frac{5}{4}-1\cdot1=\frac{1}{4}  
$$
...
- 이제 공분산의 성질을 이용하여 $\text{Cov}(X,Y)$를 다시 계산해 보자.
- 앞에서와 같이 $X_i$를 $i$번째 동전 던지기에서 앞면이 나오면 $1$, 뒷면이 나오면 $0$인 변수라고 하자. 그러면 $X_i\sim\text{Bernoulli}(0.5)$이다.
...
$$  
\text{Cov}(X,Y)=\text{Var}(X_2)=\frac{1}{4}  
$$
...
#### Discussion
- 이 예제는  $\text{Cov}(X,Y)=0$  이라고 해서 $X$와 $Y$가 독립임을 의미하지는 않음을 보여준다.
	- 실제로 $X$와 $X^2$는 매우 강하게 의존한다.
	- 왜냐하면 $X$의 값을 알면 $X^2$의 값도 100% 확실하게 알 수 있기 때문이다.
- 핵심은 공분산 $\text{Cov}(X,Y)$가 $X$와 $Y$ 사이의 선형 관계(linear relationship)를 측정한다는 점이다.
...
$$  
\text{Cov}(X,Y)=\int_0^1\int_0^1 \left(x-\frac{13}{20}\right)\left(y-\frac{13}{20}\right)f(x,y)\,dx\,dy  
$$  
...
### Correlation
- 공분산 $\text{Cov}(X,Y)$의 단위는 ‘$X$의 단위 $\times$ $Y$의 단위’이다.
...
$$  
0=\text{Var}\left(\frac{X}{\sigma_X}-\frac{Y}{\sigma_Y}\right)  
$$
...
- $X$와 $Y$가 단위정사각형 $[0,1]\times[0,1]$에서 정의된 결합확률변수이고, 결합 pdf가  $f(x,y)=2x^3+2y^3$일때, 상관계수 $\text{Cor}(X,Y)$를 구하라  
...
$$  
\text{Cor}(X,Y)=-\frac{9}{31}\approx -0.29  
$$
