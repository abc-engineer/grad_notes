...
- 데이터(Data): 실험의 결과이다.    
- 이 경우 데이터는 "앞면 55번"이다.    
- 관심 있는 파라미터터(Parameter(s) of interest): 우리는 미지의 모수 $p$의 값에 관심이 있다.    
...
- 아래로 볼록인 함수는 미분했을때 0인 p 값이 최대값을 갖음.
- 아래로 볼록인지 알려면 식을 두번 미분한 값이 < 0 이면 아래로 볼록
...
- pair $(\mu,\sigma^2)$의 최대우도추정량(MLE)을 구하여라.
#### Solution
...
$$f(x_1,\dots,x_n\mid \mu,\sigma)=\left(\frac{1}{\sqrt{2\pi}\sigma}\right)^n e^{-\sum_{i=1}^{n}\frac{(x_i-\mu)^2}{2\sigma^2}}$$
...
$$\ln\!\big(f(x_1,\dots,x_n\mid \mu,\sigma)\big)=-n\ln(\sqrt{2\pi})-n\ln(\sigma)-\sum_{i=1}^{n}\frac{(x_i-\mu)^2}{2\sigma^2}$$
...
$$\frac{\partial}{\partial \mu}\ln(f(x_1,\dots,x_n\mid \mu,\sigma))=\sum_{i=1}^{n}\frac{(x_i-\mu)}{\sigma^2}=0$$  
...
- 표본이 $n$개 있으면  $X_1,X_2,\ldots,X_n$  이 모두 독립이고 같은 분포(i.i.d.)라고 가정하면,  결합밀도는 각 밀도의 곱입니다.  

$$  
f(x_1,\ldots,x_n\mid a,b)=\prod_{i=1}^n f(x_i\mid a,b)  
$$

- 각 항이 전부 같은 값  모든 $x_i$가 $[a,b]$ 안에 있다면 각 항은 전부  
...
- $Aa$의 확률 앞에 $2$를 곱하는 이유는 **같은 유전자형이 만들어지는 경우가 두 가지** 있기 때문입니다.  
- 부모가 각각 대립유전자를 하나씩 줄 때, $A$의 빈도가 $\theta$, $a$의 빈도가 $1-\theta$라면:
...
#### 참고
조합 상수항이 생기는 이유는 **같은 개수 조합을 만드는 표본의 배열 순서가 매우 많기 때문**입니다.  
즉, 확률은 각 관측값의 확률만 곱하는 것이 아니라, **그러한 결과가 나오는 모든 순서를 합쳐야** 합니다.  
예를 들어 세 범주가 있고 각 개수가
...
- 첫 번째 범주가 들어갈 자리를 $k_1$개 고릅니다.

$$  
\binom{n}{k_1}  
$$

- 남은 $n-k_1=k_2+k_3$ 자리 중 두 번째 범주 자리를 $k_2$개 고릅니다.  

$$  
\binom{k_2+k_3}{k_2}  
$$

- 남은 자리는 자동으로 세 번째 범주 $k_3$개입니다.  
