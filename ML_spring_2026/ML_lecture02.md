...
$$L(\boldsymbol{\theta})= \frac{1}{n} \sum_{i=1}^{n}- \log p(y^{(i)} \mid \mathbf{x}^{(i)}; \boldsymbol{\theta}) 
= \frac{1}{n} \sum_{i=1}^{n}- \log \left(\frac{\exp\left(\boldsymbol{\theta}_{y^{(i)}}^T \mathbf{x}^{(i)}\right)}{\sum_{k=1}^{C} \exp\left(\boldsymbol{\theta}_k^T mathbf{x}^{(i)}\right)}\right)  
$$
...
- 트레이닝만 하면 오버 피팅이 될 수 있음
- 정규화 목적은 오버피팅 방지
- 지금까지의 흐름은 매우 명확했습니다. 우리는 모델을 정의하고, 데이터에 얼마나 잘 맞는지를 측정하는 비용 함수를 선택한 뒤, 그 비용을 최소화했습니다.
- 선형 회귀에서는 제곱 오차를 사용했고, 로지스틱 회귀에서는 확률적 관점에서 유도된 크로스 엔트로피를 사용했습니다.
- 두 경우 모두 목표는 동일했습니다. 즉, 학습 데이터가 최대한 그럴듯하게 나타나도록, 다시 말해 데이터를 가장 잘 설명할 수 있는 파라미터를 찾는 것입니다.
- 그러나 이 목적에는 미묘한 문제가 있습니다.
- 앞서 논의했듯이, 학습 오차만을 최소화하는 데 집중하면 모델이 데이터의 실제 패턴뿐만 아니라 노이즈까지 함께 학습하게 될 수 있습니다. 이를 과적합이라고 합니다.
- 이 점은 머신러닝의 핵심적인 과제 중 하나를 다시 강조합니다. 즉, 단순히 주어진 데이터를 잘 맞추는 것이 아니라, 보지 못한 새로운 데이터에도 잘 일반화하는 것이 중요하다는 것입니다.
- 모델을 좀 더 간단하게하면 오버 피팅 방지 할 수 있음
- 오버피팅은 모델이 너무 복잡해서 발생 할 수 있음
- 어떻게 간단하게 하냐? -> 오브젝티브 펑션 수정 -> 어떻게 수정?
	- 단순히 패널티 텀 g(𝜽)를 넣어주면 됨.
	- 그럼 새로운 J(𝜽) 함수를 얻을 수 있음
	- 𝜆는 그냥 상수 𝜆가 줄어들면 모델은 단순해지고 오버피팅 방지, 람다가 늘어나면 모델이 더 잘맞춤
- 학습 문제는 트레이드 오프로 바뀜
- 이런 아이디어를 regularization이라고 함.
- 𝜆가 0 이면(g_패널티 텀이 0) 괴장히 오버피팅 + 노이즈 영향 큼
	- 𝜆가 적당하면 일반화 성능이 올라감

- g(𝜽)은 어떤 함수일까?

### L1 regularization
- 정규화 기법 중 가장 흔한게 L1 regularization, (lasso regularization)
- L1은 값에 절대값을 씌우고 더해주는 값임. -> 이게 penalty term이 됨.

- L1 정규화 기법은 sparsity 을 encourages 하는 특징이 있음
	- 파라미터를 희소하게 만드는 특징이 있다.
	- 100개의 디멘젼이 있는 모델일때 몇개를 0으로 만들어 버림.
	- penalty term을 쎄게 주면 대부분은 0만 남고 중요한 피쳐 몃개만 살아남음. 
	- 이렇게 모델을 단순화 시켜서 오버피팅을 방지
- 즉 필요 없는 피쳐는 없애 버림
- L1 정규화의 주요 효과는 일부 파라미터를 정확히 0으로 만들어 해를 희소하게 만드는 것이다.
- 이는 특히 특징 선택 문제에서 유용한데, L1 정규화는 관련 없는 특징들을 효과적으로 제거하기 때문이다.
- 보통은 L1 정규화를 그림으로 그리면 사각형이 되는데 이중 꼭지점이 최적의 답에 가까운경우가 많음 그런데 꼭지점은 0,c 이런 식으로 값 하나가 0임 → 최적해가 0을 포함하는 경우가 많다.
	- $|w_1| + |w_2| \leq c$
	- 마름모의 꼭짓점
	- (0, c) (c, 0) (0, -c) (-c, 0)


### visualization of L1 regularization in 2D. 
- 왜 sparsity을 증가 시키는지를 설명하는 그림 임.
- 원래 수식
- $\frac{1}{N} \sum_{i=1}^{N} \left( y^{(i)} - \hat{y}^{(i)} \right)^2$
- L1 정규화 수식
- $\frac{1}{N} \sum_{i=1}^{N} \left( y^{(i)} - \hat{y}^{(i)} \right)^2 + \lambda \|w\|_1$
- 원래 답은 minimize loss 값인 저 과녁임.
- 하지만 패널티를 원래 minimize 함수에 더하면 과녁이 아닌 패널티와 과녁값 사이 최소값을 결정하는 코너의 값을 갖게 됨.
- 최적해는 이 꼭짓점(corner)에 걸릴 확률이 높음  
	→ 좌표 하나가 정확히 0
- 그림의 타원: loss function (오차 함수)의 등고선입니다
	- 중심으로 갈수록 loss가 작음
	- 진짜 답쪽으로 가는 등고선
- 마름모 (diamond shape): L1 penalty (‖w‖₁ ≤ c)의 제약 영역
	- $|w_1| + |w_2| \leq c$
- 정규화 하면 원래 loss만 최소화한 해보다 값은 더 나빠짐
- 하지만 일반화 성능은 오히려 좋아질 수 있기 때문에 그렇게 하는 것

### L2 regularization
- 더많이 사용하는것이 L2 정규화
- $||𝜽||_2$ 는 L2 norm(Euclidean norm) 
- L2 정규화는 𝜽제곱의 합을 패널티 텀으로 사용.
- 𝜽나 w나 같은 뜻 둘다 가중치
- L2는 파라미터 값을 더 작게 만듦.
- 다이아 몬드가 아니라 원을 그림.
...
- L2
	- 전체의 파라미터를 줄임.
	- 모델의 민감성을 줄이는 역할
	- w 값이 크면 인풋이 조금만 커도 아웃풋이 매우 커짐.
	- 리찌 정규화 ridge regularization
- 요약하면, L1과 L2 정규화는 모두 모델이 훈련 데이터에 지나치게 맞추는 것(과적합)을 방지하여 최적화 과정을 유도하는 데 도움을 준다. 
- 이러한 패널티를 도입함으로써, 최적화 문제는 단순히 손실 함수 ( f(x) )를 최소화하는 것이 아니라, 손실을 최소화하는 것과 모델의 복잡도를 제어하는 것 사이의 균형을 맞추는 문제로 바뀐다.
- L1은 일부 파라미터를 0으로 밀어 넣는 방식으로 최적화 문제를 변화시켜 더 단순한 모델을 만든다.
- 반면 L2는 파라미터 값을 전반적으로 줄이는 방식으로 문제를 변화시키며, 어떤 특징도 제거하지 않으면서 데이터의 작은 변동에 대한 모델의 민감도를 낮추는 데 도움을 준다.
